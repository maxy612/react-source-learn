...
export function updateContainer(
  ...
  const current = container.current; // 找到fiberNode
  const currentTime = requestCurrentTimeForUpdate(); // 获取当前时间（不同于Date.now，有偏移）
  if (__DEV__) {
  ...
  const suspenseConfig = requestCurrentSuspenseConfig(); // 获取suspenseConfig配置 null
  const expirationTime = computeExpirationForFiber(
    currentTime,
    current,
    suspenseConfig,
  ); // 计算expirationTime  首次时是2^30

  const context = getContextForSubtree(parentComponent); // 从父组件中获取到上下文context, 首次时为{}空对象
  if (container.context === null) { // 首次执行时 fiberRoot的上下文是Null
    container.context = context;
  } else {
    container.pendingContext = context; // 设置pendingContext（待定上下文）
  }
  // container.context = {}
  ...
  const update = createUpdate(expirationTime, suspenseConfig); // 根据执行结束时间和suspense配置创建参数
  /**
   * expirationTime,
    suspenseConfig,
    tag: UpdateState,
  ...
  update.payload = {element}; // 给update增加payload。为element，即当前reactnodelist.
  ...
  if (callback !== null) {
    ...
    update.callback = callback; // 给update增加callback方法(就是调用该函数时传入的callback)
  }

  enqueueUpdate(current, update); // 调用enqueueUpdate, 传入current(FiberNode实例)和update。和fiber的updateQueue属性相关
  // 1. 如果current.updateQueue为null, 直接返回
  // 2. current.updateQueue.shared.pending = update, update.next = update/update.next = pending.next

  scheduleUpdateOnFiber(current, expirationTime); // 在fiber上执行调度更新,

  return expirationTime; // 返回结束的时间
}
...
export function getPublicRootInstance(
  ...
  const containerFiber = container.current; // 获取FiberNode实例
  if (!containerFiber.child) { // 初始化的时候是Null
    return null;
  }
  switch (containerFiber.child.tag) {
    ...
    default:
      return containerFiber.child.stateNode; // FiberRootNode
  }
}
