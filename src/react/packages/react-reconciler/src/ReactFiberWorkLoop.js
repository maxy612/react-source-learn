...
export function computeExpirationForFiber(
  ...
  if ((mode & BlockingMode) === NoMode) { // 首次会走到这里
    return Sync; // 2^30
  }

  const priorityLevel = getCurrentPriorityLevel(); // 获取当前任务的优先级
  if ((mode & ConcurrentMode) === NoMode) {
    return priorityLevel === ImmediatePriority ? Sync : Batched; // 2^30 || 2^30 - 1
  }
...
export function scheduleUpdateOnFiber(
  ...
  expirationTime: ExpirationTime,
    ...
    checkForNestedUpdates(); // 检查非正常调用 即在componentWillUpdate或者render中执行setState，导致积压的待更新任务过多，这个时候需要给开发者提示
    debugger;
    warnAboutRenderPhaseUpdatesInDEV(fiber); // 

    // 更新fiber的各种时间(expirationTime, alternate.expiration, return.childExpirationTime, return.alternate.childExpirationTime,
    // firstPendingTime, firstSuspendedTime和lastPingedTime)
    ...
    checkForInterruption(fiber, expirationTime); // 检查更新到期时间和渲染到期时间
    recordScheduleUpdate(); // 更新hasScheduledUpdateInCurrentCommit和hasScheduledUpdateInCurrentPhase
    ...
    if (expirationTime === Sync) { // 最高优先级
      if (
        // Check if we're inside unbatchedUpdates // 在首次创建的时候会在unbatchedUpdates中执行
...
function markUpdateTimeFromFiberToRoot(fiber, expirationTime) {
  ...
  if (fiber.expirationTime < expirationTime) { // 如果fiberNode的expirationTime小于当前实际的expirationTime, 更新fiberNode的时间
    fiber.expirationTime = expirationTime;
  }
  let alternate = fiber.alternate; // null
  if (alternate !== null && alternate.expirationTime < expirationTime) {
    alternate.expirationTime = expirationTime; // 更新fiber.alternate.expirationTime
  }
  // Walk the parent path to the root and update the child expiration time.
  let node = fiber.return;
  let root = null;
  if (node === null && fiber.tag === HostRoot) {
    root = fiber.stateNode; // 也就是最父级的fiberRootNode
  } else {
    while (node !== null) {
      ...
      if (node.return === null && node.tag === HostRoot) {
        root = node.stateNode; // FiberRootNode
        break;
      }
      node = node.return;
    }
  ...
  if (root !== null) {
    if (workInProgressRoot === root) {
      ...
      markUnprocessedUpdateTime(expirationTime); // 更新workInProgressRootNextUnprocessedUpdateTime，如果它小于expirationTime
      ...
      if (workInProgressRootExitStatus === RootSuspendedWithDelay) {
        ...
        markRootSuspendedAtTime(root, renderExpirationTime); // 更新 firstSuspendedTime和lastPingedTime
      }
    }
