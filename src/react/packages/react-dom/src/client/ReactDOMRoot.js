...
function createRootImpl(
  ...
  const hydrate = options != null && options.hydrate === true; // true
  const hydrationCallbacks =
    (options != null && options.hydrationOptions) || null; // null
  const root = createContainer(container, tag, hydrate, hydrationCallbacks); // 创建fiberRoot实例
  markContainerAsRoot(root.current, container); // 将实例标记为 root(根实例)
